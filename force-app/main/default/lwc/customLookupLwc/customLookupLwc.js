...
import getNameField from '@salesforce/apex/CustomLookupLwcController.getNameField';
...
export default class CustomLookupLwc extends LightningElement {
    ...
    connectedCallback() { 
        ...
        if (this.defaultRecordId != '' && (this.selectedRecord == undefined || this.selectedRecord == {} || this.selectedRecord.Id == undefined)) {
            this.fetchRecord();
        } else {
    ...
    onSelectRecordHandler(event) {
        var selectedRecord = JSON.parse(event.detail);
        this.lookupUpdatehandler(selectedRecord); // update value on parent component as well from helper function 
        this.handelSelectRecordHelper(); // helper function to show/hide lookup result container on UI
    }
